...
void Init_MMA8451Q()
{
	...
	// Debug Messages
	LOG("\r\n");
	LOG("\r\n MMA8451Q Accelerometer is initializing ...");
...
//    MMA8451Q_SetTransient();
    ...
    LOG("\r\n --- MMA8451Q Accelerometer configuration is complete --- ");
    LOG("\r\n");
#endif
    Led_Down();
}
...
void convert_xyz_to_roll_pitch(mma8451q_acc_t *acc, float *roll, float *pitch) {
	float ax = acc->xyz[0]/SENSITIVITY_2G,
				ay = acc->xyz[1]/SENSITIVITY_2G,
				az = acc->xyz[2]/SENSITIVITY_2G;
...
void get_axValue(mma8451q_acc_t *acc, float *new_Accelvalue_ax)
{
	float ax_1 =  acc->xyz[0];
	float ax_2 = ax_1/SENSITIVITY_2G;

	*new_Accelvalue_ax = ax_2;
}
void get_ayValue(mma8451q_acc_t *acc, float *new_Accelvalue_ay)
{
	float ay_1 =  (float)acc->xyz[1];
	float ay_2 = ay_1/SENSITIVITY_2G;

	*new_Accelvalue_ay = ay_2;
}
void get_azValue(mma8451q_acc_t *acc, float *new_Accelvalue_az)
{
	float az_1 =  acc->xyz[2];
	float az_2 = az_1/SENSITIVITY_2G;

	*new_Accelvalue_az = az_2;
}
void skiddetection(float new_Accelvalue_ax, int *skid_detected)
{
 float Ax_wheelsensor[9]={0, 0, 0, 0, 0, 0, 0, 0, 0};
 int ax = new_Accelvalue_ax;
 for(int i=0;i<=9;i++)
 {
	 if(Ax_wheelsensor[i]!=ax)
	 {
		 *skid_detected=1;
	 }
	 else
...
void velocitycalulation(float new_Accelvalue_ax, int *velocity)
{
 float ax = new_Accelvalue_ax;
 int V=0,U=0;
 int firstcalc=1;
 if(flag_velocity==1)
 {
